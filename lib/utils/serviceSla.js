...
export function getServiceSla(request, now = Date.now()) {
    ...
    const storedDueDate = request?.sla_due_at ? new Date(request.sla_due_at) : null;
    const dueDate = storedDueDate && !Number.isNaN(storedDueDate.getTime())
        ? storedDueDate
        : new Date(createdTime + targetDays * DAY_MS);
    const remainingDays = Math.ceil((dueDate.getTime() - now) / DAY_MS);
    ...
    return {
        ...
        state,
        targetDays,
        escalationLevel: Number(request?.escalation_level || 0),
        breachedAt: request?.sla_breached_at || null
    };
}
