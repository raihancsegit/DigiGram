...
export const adminService = {
    ...
    toggleService: async (locationId, serviceId, isActive) => {
        const response = await authenticatedFetch('/api/admin/mutate-service', {
            method: 'POST',
    ...
    createLocation: async (locationData) => {
        const response = await authenticatedFetch('/api/admin/mutate-location', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
    ...
    mutateUser: async (id, action, updates = {}) => {
        const response = await authenticatedFetch('/api/admin/mutate-user', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
    ...
    getMigrationAudit: async () => {
        ...
        const result = await response.json();
        if (!response.ok) throw new Error(result.error || 'Migration audit failed');
        return result.data;
    },
    ...
    getDashboardOverview: async () => {
        ...
        const result = await response.json();
        if (!response.ok) throw new Error(result.error || 'Dashboard overview failed');
        return result;
    },

    seedSchoolDemo: async (institutionId) => {
        ...
        const result = await response.json();
        if (!response.ok) {
            const details = [result.error, result.hint].filter(Boolean).join(' | ');
            throw new Error(details || 'School demo seed failed');
        }
        return result.data;
    }
};
