...
import { repairMojibakeValue } from '../utils/textEncoding';
...
export const marketService = {
    ...
    getCommodities: async () => {
        ...
        return repairMojibakeValue(data);
    },
    ...
    getGlobalMarketOverview: async () => {
        ...
        const { data: markets, error: mError } = await supabase
            .from('markets')
            .select('id, name, type, location_id, union:locations(name_bn, name_en, slug)')
            .eq('is_active', true);
        ...
        return repairMojibakeValue({ markets, prices });
    },
    ...
    updatePrice: async ({ marketId, commodityId, price, supply, updatedBy }) => {
        const response = await authenticatedFetch('/api/admin/mutate-market', {
            method: 'POST',
    ...
    getMarketComplaints: async ({ locationId, marketId, limit = 30 }) => {
        const params = new URLSearchParams();
        if (locationId) params.set('locationId', locationId);
        if (marketId) params.set('marketId', marketId);
        params.set('limit', String(limit));

        const response = await authenticatedFetch(`/api/market/complaints?${params.toString()}`);
        const result = await response.json();
        if (!response.ok) throw new Error(result.error || 'Complaint load failed');
        return repairMojibakeValue(result.data || []);
    },

    updateMarketComplaint: async ({ id, status, officerNote, reviewedBy }) => {
        const response = await authenticatedFetch('/api/market/complaints', {
            method: 'PATCH',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ id, status, officerNote, reviewedBy })
        });

        const result = await response.json();
        if (!response.ok) throw new Error(result.error || 'Complaint update failed');
        return result.data;
    }
};
