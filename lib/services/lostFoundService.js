...
export const lostFoundService = {
    ...
    getPostById: async (id) => {
        ...
        if (error) throw error;
        return data ? repairMojibakeValue(data) : null;
    },

    // Get only global posts with Pagination and Filtering
    getGlobalPosts: async (page = 1, pageSize = 12, timeframe = 'all', searchQuery = '', typeFilter = 'all', returnMeta = false) => {
        const from = (page - 1) * pageSize;
        const to = from + pageSize - 1;
        ...
        if (search) {
            ...
            query = query.or(`title.ilike.%${q}%,description.ilike.%${q}%,contact_name.ilike.%${q}%,contact_phone.ilike.%${q}%`);
        }
