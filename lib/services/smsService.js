...
export const smsService = {
    ...
    async getGatewayBalance(gatewayId) {
        const response = await authenticatedFetch('/api/admin/sms', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ action: 'gateway_balance', gatewayId })
        });
        const result = await response.json();
        if (!response.ok) throw new Error(result.error || 'Gateway balance check failed');
        return result.data;
    },
