...
import { buildDemoHouseholdsForVillage, buildDemoVillageStats, buildDemoWardStats } from '@/lib/utils/demoHouseholds';

function isDemoWardId(value) {
    return String(value || '').startsWith('demo-ward');
}

function isDemoVillageId(value) {
    return String(value || '').startsWith('demo-village');
}

function getDemoWardSuffix(wardId) {
    return String(wardId || '').replace(/^demo-ward-\d+-?/, '') || '1';
}

function buildDemoHouseholdVillageRows(wardId) {
    const suffix = getDemoWardSuffix(wardId);
    return [
        { key: 'a', name: 'Demo Village A', bn_name: 'ডেমো গ্রাম ক' },
        { key: 'b', name: 'Demo Village B', bn_name: 'ডেমো গ্রাম খ' }
    ].map((village) => {
        const id = `demo-village-${village.key}-${suffix}`;
        const stats = buildDemoVillageStats(id);
        return {
            id,
            ward_id: wardId,
            name: village.name,
            ...
            location_id: id,
            location_village_id: id,
            total_estimated_houses: stats.total_houses,
            stats
        };
    });
}

function normalizeDemoHouseholdForManager(household) {
    return {
        ...household,
        service_requests: [],
        household_taxes: []
...
export const householdService = {
    ...
    async getVillagesByWard(wardId) {
        if (isDemoWardId(wardId)) return buildDemoHouseholdVillageRows(wardId);
    ...
    async getLocationVillagesByWard(wardId) {
        if (isDemoWardId(wardId)) {
            return buildDemoHouseholdVillageRows(wardId).map((village) => ({
                id: village.id,
                name_bn: village.bn_name,
    ...
    async getOrCreateVillageForLocation(wardId, locationVillage) {
        ...
        if (isDemoVillageId(locationVillage.id)) {
            return {
                ...locationVillage,
                id: locationVillage.id,
                ward_id: wardId,
                name: locationVillage.name_en || locationVillage.name || 'Demo Village',
                bn_name: locationVillage.name_bn || locationVillage.name || 'ডেমো গ্রাম',
                location_id: locationVillage.id,
                location_village_id: locationVillage.id
            };
        }
    ...
    async getHouseholdsByVillage(villageId) {
        if (isDemoVillageId(villageId)) {
            return buildDemoHouseholdsForVillage(villageId).map(normalizeDemoHouseholdForManager);
        }
    ...
    async getHouseholdStats(wardId) {
        if (isDemoWardId(wardId)) {
            const stats = buildDemoWardStats(wardId);
            return {
