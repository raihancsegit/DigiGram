...
function isFemaleResident(resident = {}) {
    ...
    return ['female', 'woman', 'à¦¨à¦¾à¦°à§€', 'à¦®à¦¹à¦¿à¦²à¦¾', 'à¦®à§‡à§Ÿà§‡'].includes(gender);
}

function isWidowResident(resident = {}) {
    const marital = String(resident.marital_status || '').toLowerCase();
    return marital.includes('widow') || marital.includes('à¦¬à¦¿à¦§à¦¬à¦¾');
}
...
const SUGGESTED_REQUEST_TYPES = {
    nid_application: {
        title: 'NID à¦†à¦¬à§‡à¦¦à¦¨ follow-up',
        details: 'Citizen quality dashboard à¦¥à§‡à¦•à§‡ NID à¦†à¦¬à§‡à¦¦à¦¨ follow-up à¦¤à§ˆà¦°à¦¿ à¦•à¦°à¦¾ à¦¹à§Ÿà§‡à¦›à§‡à¥¤'
    },
    birth_registration: {
        title: 'à¦œà¦¨à§à¦® à¦¨à¦¿à¦¬à¦¨à§à¦§à¦¨ à¦†à¦¬à§‡à¦¦à¦¨ follow-up',
        details: 'Citizen quality dashboard à¦¥à§‡à¦•à§‡ à¦œà¦¨à§à¦® à¦¨à¦¿à¦¬à¦¨à§à¦§à¦¨ à¦†à¦¬à§‡à¦¦à¦¨ follow-up à¦¤à§ˆà¦°à¦¿ à¦•à¦°à¦¾ à¦¹à§Ÿà§‡à¦›à§‡à¥¤'
    },
    widow_allowance: {
        title: 'à¦¬à¦¿à¦§à¦¬à¦¾ à¦­à¦¾à¦¤à¦¾ à¦¯à¦¾à¦šà¦¾à¦‡',
        details: 'Women Support Desk à¦¥à§‡à¦•à§‡ à¦¬à¦¿à¦§à¦¬à¦¾ à¦­à¦¾à¦¤à¦¾ à¦¯à¦¾à¦šà¦¾à¦‡ case à¦¤à§ˆà¦°à¦¿ à¦•à¦°à¦¾ à¦¹à§Ÿà§‡à¦›à§‡à¥¤ à¦ªà§à¦°à§Ÿà§‹à¦œà¦¨à§€à§Ÿ à¦•à¦¾à¦—à¦œà¦ªà¦¤à§à¦° à¦“ à¦¯à§‹à¦—à§à¦¯à¦¤à¦¾ à¦¯à¦¾à¦šà¦¾à¦‡ à¦•à¦°à§à¦¨à¥¤'
    },
    maternity_support: {
        title: 'à¦®à¦¾à¦¤à§ƒà¦¤à§à¦¬à¦•à¦¾à¦²à§€à¦¨ à¦¸à¦¹à¦¾à§Ÿà¦¤à¦¾ follow-up',
        details: 'Women Support Desk à¦¥à§‡à¦•à§‡ à¦®à¦¾à¦¤à§ƒà¦¤à§à¦¬à¦•à¦¾à¦²à§€à¦¨ à¦¸à§à¦¬à¦¾à¦¸à§à¦¥à§à¦¯/à¦¸à¦¹à¦¾à§Ÿà¦¤à¦¾ follow-up case à¦¤à§ˆà¦°à¦¿ à¦•à¦°à¦¾ à¦¹à§Ÿà§‡à¦›à§‡à¥¤'
    },
    women_health_checkup: {
        title: 'à¦¨à¦¾à¦°à§€ à¦¸à§à¦¬à¦¾à¦¸à§à¦¥à§à¦¯ checkup follow-up',
        details: 'Women Support Desk à¦¥à§‡à¦•à§‡ à¦¸à§à¦¬à¦¾à¦¸à§à¦¥à§à¦¯ checkup à¦“ blood group update follow-up à¦¤à§ˆà¦°à¦¿ à¦•à¦°à¦¾ à¦¹à§Ÿà§‡à¦›à§‡à¥¤'
    }
};

export const householdService = {
    async adminHouseholdAction(action, payload = {}) {
        const response = await authenticatedFetch('/api/admin/household-records', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ action, ...payload })
        });
        const result = await response.json();
        if (!response.ok) throw new Error(result.error || 'Admin household action failed');
        return result.data;
    },
    ...
    async syncHouseholdStats(householdId) {
        ...
        const stats = (residents || []).reduce((acc, r) => {
            ...
            return {
                ...
                male_voters: acc.male_voters + (r.is_voter && isMaleGender(r.gender) ? 1 : 0),
                female_voters: acc.female_voters + (r.is_voter && isFemaleGender(r.gender) ? 1 : 0),
                males: acc.males + (isMaleGender(r.gender) ? 1 : 0),
                females: acc.females + (isFemaleGender(r.gender) ? 1 : 0),
    ...
    async getHouseholdLockerServiceRequests(idOrCode, pin) {
        const isValid = await this.verifyLockerPin(idOrCode, pin);
        if (!isValid) throw new Error('Invalid Locker PIN');

        const { data, error } = await supabase.rpc('get_household_locker_service_requests', {
            lookup_value: idOrCode,
            candidate_pin: pin
        });
        ...
        console.warn('Locker service request RPC unavailable, falling back to direct query:', error.message);

        const isUuid = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(String(idOrCode || ''));
        let householdQuery = supabase
            .from('households')
            .select('id');
        householdQuery = isUuid ? householdQuery.or(`id.eq.${idOrCode},qr_code_id.eq.${idOrCode}`) : householdQuery.eq('qr_code_id', idOrCode);
        ...
        if (requestError) {
            console.warn('Direct locker service request query skipped:', requestError.message);
            return [];
        }
    ...
    async getCitizenQualityDashboardByUnion(unionId) {
        ...
        const candidates = residents.flatMap((resident) => {
            ...
            if (age !== null && age >= 18 && !resident.nid) {
                next.push({
                    ...
                    label: 'NID à¦†à¦¬à§‡à¦¦à¦¨ à¦ªà§à¦°à§Ÿà§‹à¦œà¦¨',
            ...
            if (age !== null && age < 18 && !resident.birth_reg_no) {
                next.push({
                    ...
                    label: 'à¦œà¦¨à§à¦® à¦¨à¦¿à¦¬à¦¨à§à¦§à¦¨ à¦ªà§à¦°à§Ÿà§‹à¦œà¦¨',
            ...
            if (!resident.blood_group) {
                next.push({
                    ...
                    label: 'à¦°à¦•à§à¦¤à§‡à¦° à¦—à§à¦°à§à¦ª à¦†à¦ªà¦¡à§‡à¦Ÿ à¦ªà§à¦°à§Ÿà§‹à¦œà¦¨',
                    activeRequest: null
                });
            }
            return next;
        ...
        const womenSupport = residents.flatMap((resident) => {
            ...
            const next = [];
            if (isWidowResident(resident)) next.push({ ...base, type: 'widow_allowance', label: 'à¦¬à¦¿à¦§à¦¬à¦¾ à¦­à¦¾à¦¤à¦¾ à¦¯à¦¾à¦šà¦¾à¦‡', priority: 'high', activeRequest: activeRequestMap.get(`${resident.id}:widow_allowance`) || null });
            if (age !== null && age >= 13 && age <= 49) next.push({ ...base, type: 'maternity_support', label: 'à¦®à¦¾à¦¤à§ƒà¦¤à§à¦¬/à¦¨à¦¾à¦°à§€ à¦¸à§à¦¬à¦¾à¦¸à§à¦¥à§à¦¯ follow-up', priority: 'normal', activeRequest: activeRequestMap.get(`${resident.id}:maternity_support`) || null });
            if (!resident.blood_group) next.push({ ...base, type: 'women_health_checkup', label: 'à¦¸à§à¦¬à¦¾à¦¸à§à¦¥à§à¦¯ checkup à¦“ blood group update', priority: 'normal', activeRequest: activeRequestMap.get(`${resident.id}:women_health_checkup`) || null });
            return next;
        }).slice(0, 30);
    ...
    async updateRequestStatus(requestId, status, collectionDate, feedback, unionId = null) {
        ...
        const certificateNo = (
            ...
            ? `${existingRequest?.request_type === 'death_certificate' ? 'D' : existingRequest?.request_type === 'warish_certificate' ? 'W' : 'B'}-${new Date().getFullYear()}-${requestId.slice(0, 8).toUpperCase()}`
            : existingRequest?.certificate_no;
