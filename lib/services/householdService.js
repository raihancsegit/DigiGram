...
export const householdService = {
    ...
    async createServiceRequest(requestData) {
        ...
        await fetch('/api/service-requests/notify', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ requestId: data.id, eventKey: 'submitted' })
        });
    ...
    async createSuggestedServiceRequestForResident(residentId, requestType, unionId) {
        ...
        if (!['nid_application', 'birth_registration'].includes(requestType)) {
            throw new Error('This suggestion does not support automatic application creation');
        }
        ...
        const request = await this.createServiceRequest({
            ...
            details: requestType === 'nid_application'
                ? 'Citizen quality dashboard থেকে NID আবেদন তৈরি করা হয়েছে।'
                : 'Citizen quality dashboard থেকে জন্ম নিবন্ধন আবেদন তৈরি করা হয়েছে।',
            status: 'pending',
            meta_data: {
    ...
    async getCitizenQualityDashboardByUnion(unionId) {
        if (!unionId) {
            return {
                summary: { totalResidents: 0, missingNid: 0, missingBirthReg: 0, missingBloodGroup: 0, voterEligibleWithoutNid: 0, childrenWithoutBirthReg: 0 },
                wardSummaries: [],
                candidates: []
            };
        }
        ...
        const summary = residents.reduce((acc, resident) => {
            ...
            return acc;
        }, { totalResidents: 0, missingNid: 0, missingBirthReg: 0, missingBloodGroup: 0, voterEligibleWithoutNid: 0, childrenWithoutBirthReg: 0 });
    ...
    async updateRequestStatus(requestId, status, collectionDate, feedback, unionId = null) {
        ...
        const { data: existingRequest } = await supabase
            .from('service_requests')
            .select('resident_id, request_type, certificate_no')
            .eq('id', requestId)
            .single();
        ...
        if (['processing', 'ready', 'rejected', 'completed'].includes(normalizedStatus)) {
            await fetch('/api/service-requests/notify', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ requestId, eventKey: normalizedStatus })
            });
        }
