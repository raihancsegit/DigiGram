...
export const verifyOperatorPassword = async (unionSlug, password) => {
    const { data: verified, error: rpcError } = await supabase.rpc('verify_fuel_operator_password', {
        target_union_slug: unionSlug,
        candidate_password: String(password || '')
    });
    if (!rpcError) return Boolean(verified);
