...
export const schoolService = {
    ...
    async createStudent(payload) {
        ...
        if (payload.resident_id) {
            const { error: residentError } = await supabase
                .from('residents')
                .update({
                    student_status: payload.enrollment_status || 'studying',
                    current_institution_id: payload.institution_id,
                    current_class_name: payload.class_name || null,
                    current_roll_no: payload.roll_no || null,
                    school_enrollment_updated_at: new Date().toISOString()
                })
                .eq('id', payload.resident_id);
            if (residentError) console.error('Resident school enrollment update failed:', residentError);
        }

        return data;
    },

    async searchHouseholdMembers(institutionId, query) {
        const params = new URLSearchParams({ q: query });
        const response = await authenticatedFetch(`/api/institutions/${encodeURIComponent(institutionId)}/household-members?${params.toString()}`, {
            cache: 'no-store'
        });
        const body = await response.json();
        if (!response.ok) throw new Error(body.error || 'Household member search failed');
        return body.data?.households || [];
    },
    ...
    async getStudentTransitions(studentId) {
        const { data, error } = await supabase
            .from('school_student_transitions')
            .select('*')
    ...
    async transitionStudent(studentId, transitionType, targetClassId = null, reason = null, effectiveDate = null) {
        const { data, error } = await supabase.rpc('transition_school_student', {
            target_student_id: studentId,
            transition_name: transitionType,
            target_class_id: targetClassId,
            transition_reason: reason,
            transition_date: effectiveDate || new Date().toISOString().split('T')[0]
        });
        if (error) throw error;
        return data;
    },
    ...
    async getClassAttendance(classId, attendanceDate) {
        let query = supabase
            .from('school_attendance')
            .select('*')
            .eq('class_id', classId)
    ...
    async getInstitutionAttendance(institutionId, dateFrom, dateTo) {
        let query = supabase
            .from('school_attendance')
            .select('*')
            .eq('institution_id', institutionId)
    ...
    async getStaffAttendance(institutionId, attendanceDate) {
        let query = supabase
    ...
    async markStaffAttendance(payload) {
        const { data, error } = await supabase
            ...
            .upsert({
                ...payload,
                updated_at: new Date().toISOString()
            }, { onConflict: 'institution_id,profile_id,attendance_date' })
            .select()
            .single();
        if (error) throw error;
        return data;
    ...
    async getAcademicSessions(institutionId) {
        const { data, error } = await supabase
            .from('school_academic_sessions')
            .select('*')
            .eq('institution_id', institutionId)
    ...
    async createAcademicSession(payload) {
        const { data, error } = await supabase
            .from('school_academic_sessions')
            .insert([payload])
            .select()
    ...
    async activateAcademicSession(sessionId) {
        const { data, error } = await supabase.rpc('activate_school_academic_session', {
    ...
    async getRoutinePeriods(institutionId) {
        const { data, error } = await supabase
    ...
    async createRoutinePeriod(payload) {
        const { data, error } = await supabase
            .from('school_routine_periods')
            .insert([payload])
            .select()
    ...
    async deleteRoutinePeriod(periodId) {
        const { error } = await supabase
            .from('school_routine_periods')
            .delete()
            .eq('id', periodId);
    ...
    async getFeeTypes(institutionId) {
        const { data, error } = await supabase
    ...
    async getFeeInvoices(institutionId) {
        const { data, error } = await supabase
    ...
    async createFeeInvoice(payload) {
        const { data, error } = await supabase
            .from('school_fee_invoices')
            .insert([payload])
            .select()
    ...
    async recordFeePayment(invoiceId, amount, paymentMethod = 'cash', transactionReference = null, note = null) {
    ...
    async getFinanceEntries(institutionId) {
        const { data, error } = await supabase
            .from('school_finance_entries')
            .select('*')
            .eq('institution_id', institutionId)
    ...
    async getStaffCompensation(institutionId) {
        const { data, error } = await supabase
    ...
    async saveStaffCompensation(payload) {
        const { data, error } = await supabase
            .from('school_staff_compensation')
            .upsert(payload, { onConflict: 'institution_id,profile_id' })
            .select()
            .single();
        if (error) throw error;
        return data;
    ...
    async getPayrollRuns(institutionId) {
        const { data, error } = await supabase
    ...
    async generatePayroll(institutionId, payrollMonth) {
        const { data, error } = await supabase.rpc('generate_school_payroll', {
            target_institution_id: institutionId,
            target_month: payrollMonth
        });
        if (error) throw error;
        return data;
    },
    ...
    async markPayrollPaid(payrollId) {
        const { data, error } = await supabase.rpc('mark_school_payroll_paid', {
            target_payroll_id: payrollId
        });
        if (error) throw error;
        return data;
    },
    ...
    async getHomeworkSubmissionsByLesson(lessonId) {
        const { data, error } = await supabase
    ...
    async getStudentHomeworkSubmissions(studentId) {
        const { data, error } = await supabase
            .from('school_homework_submissions')
            .select('*')
            .eq('student_id', studentId)
    ...
    async reviewHomework(submissionId, updates) {
        const { data, error } = await supabase
            .from('school_homework_submissions')
            .update({
                ...updates,
