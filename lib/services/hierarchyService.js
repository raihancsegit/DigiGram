...
export async function getFullContextBySlug(unionSlug) {
    ...
    return {
        union: { id: union.id, slug: union.slug, name: union.name_bn },
        upazila,
        district
    };
}
...
// 6. Fetch Village with Full Context from just Village ID
export async function getVillageFullContext(villageId) {
    const isUUID = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(villageId);
    if (!isUUID) return null;

    // 1. Get the village
    const { data: village, error: vError } = await supabase
        .from('locations')
        .select('*')
        .eq('id', villageId)
        .single();
