...
import { buildDemoHouseholdDonors, buildDemoVillageStats, buildDemoWardStats } from '@/lib/utils/demoHouseholds';
...
function buildAccurateDemoWardContext(wardIdOrSlug) {
    const value = String(wardIdOrSlug || '');
    if (!value.startsWith('demo-ward')) return null;

    const wardNumber = value.match(/demo-ward-(\d+)/)?.[1] || '1';
    const wardNumberBn = toBanglaDigit(wardNumber);
    const suffix = value.replace(/^demo-ward-\d+-?/, '') || wardNumber;
    const unionSlug = `demo-union-${suffix}`;
    const villages = [
        { key: 'a', name: 'ডেমো গ্রাম ক' },
        { key: 'b', name: 'ডেমো গ্রাম খ' }
    ].map((village) => {
        const villageId = `demo-village-${village.key}-${suffix}`;
        const stats = buildDemoVillageStats(villageId);
        return {
            id: villageId,
            slug: villageId,
            name: village.name,
            name_bn: village.name,
            population: String(stats.population),
            voters: String(stats.voters),
            maleVoters: String(stats.maleVoters),
            femaleVoters: String(stats.femaleVoters),
            total_houses: String(stats.total_houses),
            stats,
            real_stats: stats,
    ...
    const stats = buildDemoWardStats(value);
    const ward = {
        id: value,
        slug: value,
        name: `ডেমো ওয়ার্ড ${wardNumberBn}`,
        name_bn: `ডেমো ওয়ার্ড ${wardNumberBn}`,
        population: String(stats.population),
        voters: String(stats.voters),
        stats,
        real_stats: stats,
        survey_status: 'verified',
        member: { name: 'ডেমো মেম্বার', phone: '01700009999' },
        villages,
        bloodDonors: villages.flatMap((village) => buildDemoHouseholdDonors(village.id))
    };

    return {
        ward,
        ctx: {
            district: { id: `demo-district-${suffix}`, name: 'ডেমো জেলা' },
            upazila: { id: `demo-upazila-${suffix}`, name: 'ডেমো উপজেলা' },
            union: {
                id: unionSlug,
                slug: unionSlug,
                name: 'ডেমো ইউনিয়ন',
...
function buildAccurateDemoUnionContext(unionSlug) {
    const value = String(unionSlug || '');
    if (!value.startsWith('demo-union')) return null;

    const suffix = value.replace(/^demo-union-?/, '') || '1';
    const wards = [1, 2, 3]
        .map((wardNumber) => buildAccurateDemoWardContext(`demo-ward-${wardNumber}-${suffix}`)?.ward)
        .filter(Boolean);
...
function buildAccurateDemoVillageContext(villageIdOrSlug, wardIdOrSlug = null, unionSlug = null) {
    const value = String(villageIdOrSlug || '');
    if (!value.startsWith('demo-village')) return null;

    const suffix = value.replace(/^demo-village-[ab]-?/, '') || String(wardIdOrSlug || '').replace(/^demo-ward-\d+-?/, '') || '1';
    const resolvedUnionSlug = unionSlug || `demo-union-${suffix}`;
    const resolvedWardId = wardIdOrSlug || `demo-ward-1-${suffix}`;
    const ctx = buildAccurateDemoUnionContext(resolvedUnionSlug);
    if (!ctx) return null;

    const ward = ctx.union.wards.find((item) => item.id === resolvedWardId || item.slug === resolvedWardId) || ctx.union.wards[0];
    const village = ward?.villages?.find((item) => item.id === value || item.slug === value) || null;
    ...
    return {
        ...
        ward: {
            ...ward,
            villages: [village, ...(ward?.villages || []).filter((item) => item.id !== value && item.slug !== value)]
        },
        village
    };
}
...
function buildDemoWardContext(wardIdOrSlug) {
    const value = String(wardIdOrSlug || '');
    if (!value.startsWith('demo-ward')) return null;
    return buildAccurateDemoWardContext(value);
    const wardNumber = value.match(/demo-ward-(\d+)/)?.[1] || '1';
    const wardNumberBn = toBanglaDigit(wardNumber);
    const suffix = value.replace(/^demo-ward-\d+-?/, '');
    const unionSlug = suffix ? `demo-union-${suffix}` : 'demo-union';
    const ward = {
        id: value,
        slug: value,
        name: `ডেমো ওয়ার্ড ${wardNumberBn}`,
        ...
        member: { name: 'ডেমো মেম্বার', phone: '01700009999' },
        villages: [
            { id: `demo-village-a-${suffix || wardNumber}`, slug: `demo-village-a-${suffix || wardNumber}`, name: 'ডেমো গ্রাম ক', name_bn: 'ডেমো গ্রাম ক', population: '১৬০০', voters: '৯০০' },
            { id: `demo-village-b-${suffix || wardNumber}`, slug: `demo-village-b-${suffix || wardNumber}`, name: 'ডেমো গ্রাম খ', name_bn: 'ডেমো গ্রাম খ', population: '১৬০০', voters: '৯০০' }
        ],
        bloodDonors: []
    };
    ...
    return {
        ward,
        ctx: {
            district: { id: `demo-district-${suffix || wardNumber}`, name: 'ডেমো জেলা' },
            upazila: { id: `demo-upazila-${suffix || wardNumber}`, name: 'ডেমো উপজেলা' },
            union: {
                id: unionSlug,
                slug: unionSlug,
                name: 'ডেমো ইউনিয়ন',
...
export function buildDemoUnionContext(unionSlug) {
    const value = String(unionSlug || '');
    if (!value.startsWith('demo-union')) return null;
    const suffix = value.replace(/^demo-union-?/, '');
    const wardSuffix = suffix || '1';
    const wards = [1, 2, 3]
        .map((wardNumber) => buildDemoWardContext(`demo-ward-${wardNumber}-${wardSuffix}`)?.ward)
        .filter(Boolean);

    return {
        district: { id: `demo-district-${wardSuffix}`, name: 'ডেমো জেলা' },
        upazila: { id: `demo-upazila-${wardSuffix}`, name: 'ডেমো উপজেলা' },
        union: {
            id: value,
            slug: value,
            name: 'ডেমো ইউনিয়ন',
...
export function buildCleanDemoUnionContext(unionSlug) {
    const value = String(unionSlug || '');
    if (!value.startsWith('demo-union')) return null;
    return buildAccurateDemoUnionContext(value);
    const suffix = value.replace(/^demo-union-?/, '');
    const wardSuffix = suffix || '1';
    const wards = [1, 2, 3].map((wardNumber) => ({
        id: `demo-ward-${wardNumber}-${wardSuffix}`,
        slug: `demo-ward-${wardNumber}-${wardSuffix}`,
        name: `ডেমো ওয়ার্ড ${toBanglaDigit(wardNumber)}`,
        name_bn: `ডেমো ওয়ার্ড ${toBanglaDigit(wardNumber)}`,
        population: '৩২০০',
        voters: '১৮০০',
        member: { name: 'ডেমো মেম্বার', phone: '01700009999' },
        villages: [
            { id: `demo-village-a-${wardSuffix}`, slug: `demo-village-a-${wardSuffix}`, name: 'ডেমো গ্রাম ক', name_bn: 'ডেমো গ্রাম ক', population: '১৬০০', voters: '৯০০' },
            { id: `demo-village-b-${wardSuffix}`, slug: `demo-village-b-${wardSuffix}`, name: 'ডেমো গ্রাম খ', name_bn: 'ডেমো গ্রাম খ', population: '১৬০০', voters: '৯০০' }
        ],
        bloodDonors: []
    }));
...
export function buildCleanDemoVillageContext(villageIdOrSlug, wardIdOrSlug = null, unionSlug = null) {
    const value = String(villageIdOrSlug || '');
    if (!value.startsWith('demo-village')) return null;
    return buildAccurateDemoVillageContext(value, wardIdOrSlug, unionSlug);

    const suffix = value.replace(/^demo-village-[ab]-?/, '') || String(wardIdOrSlug || '').replace(/^demo-ward-\d+-?/, '') || '1';
    const villageLetter = value.includes('village-b') ? 'b' : 'a';
    const villageName = villageLetter === 'b' ? 'ডেমো গ্রাম খ' : 'ডেমো গ্রাম ক';
    const resolvedUnionSlug = unionSlug || `demo-union-${suffix}`;
    const resolvedWardId = wardIdOrSlug || `demo-ward-1-${suffix}`;
    const ctx = buildCleanDemoUnionContext(resolvedUnionSlug);
    if (!ctx) return null;
        ...
        return wards.map(ward => {
            ...
            const mappedVillages = wardVillages.map(v => {
                ...
                if (v.survey_status === 'verified' && v.real_stats) {
                    ...
                    statsObj.population = rs.total_members ?? rs.population ?? 0;
                    statsObj.total_members = rs.total_members ?? rs.population ?? 0;
                    statsObj.voters = rs.voters ?? 0;
                    statsObj.maleVoters = rs.male_voters ?? rs.maleVoters ?? 0;
                    statsObj.femaleVoters = rs.female_voters ?? rs.femaleVoters ?? 0;
                    statsObj.males = rs.males ?? 0;
                    statsObj.females = rs.females ?? 0;
                ...
                return {
                    ...
                    name_bn: v.name_bn,
                    name_en: v.name_en,
                    survey_status: v.survey_status,
                    real_stats: v.real_stats,
                    stats: v.stats,
                    total_estimated_houses: v.total_estimated_houses,
                    ...statsObj
                };
            });
...
export async function getVillageFullContext(villageIdOrSlug) {
    const demoFallback = buildCleanDemoVillageContext(villageIdOrSlug);
    ...
    if (vError || !village) return demoFallback || null;
...
export async function getWardFullContext(wardIdOrSlug) {
    const demoFallback = buildDemoWardContext(wardIdOrSlug);
    ...
    if (wError || !ward) return demoFallback || null;
