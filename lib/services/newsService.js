...
export const newsService = {
    ...
    getNews: async (locationId, page = 1, pageSize = 12, asGlobal = false, timeframe = 'all') => {
        ...
        return { data: repairMojibakeValue(data || []), count };
    },

    // Get Global News with Pagination and Filtering
    getGlobalNews: async (page = 1, pageSize = 10, timeframe = 'all', searchQuery = '', returnMeta = false) => {
        const from = (page - 1) * pageSize;
        const to = from + pageSize - 1;
        ...
        if (search) {
            ...
            query = query.or(`title.ilike.%${q}%,excerpt.ilike.%${q}%,description.ilike.%${q}%,category.ilike.%${q}%`);
        }
        ...
        if (error) throw error;
        const normalizedData = repairMojibakeValue(data || []);
        return returnMeta ? { data: normalizedData, count: count || 0 } : normalizedData;
    },
    ...
    getNewsBySlugOrId: async (identifier) => {
        ...
        try {
            ...
            if (isUuid) {
                ...
                if (data) return repairMojibakeValue(data); // Found by ID
            }
            ...
            return slugData ? repairMojibakeValue(slugData) : null;
