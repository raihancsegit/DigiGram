...
file_path = "app/(site)/admin/union/page.js"
...
"""
content = content.replace("const [errorMessage, setErrorMessage] = useState('');", "const [errorMessage, setErrorMessage] = useState('');" + state_vars)
...
"""
content = content.replace("const handleAssignChairman = async (userId) => {", ward_handlers + "\n    const handleAssignChairman = async (userId) => {")
...
content = content.replace("await adminService.assignRoleToUser(data.user.id, 'chairman', selectedUnion.id);", """
            const role = activeRoleAssign;
            const scopeId = activeRoleAssign === 'ward_member' ? activeWardForAssign.id : selectedUnion.id;
            // First UPSERT the auth profile but with dynamic role
            const { error: profileError } = await adminService.supabase // Need to avoid the local adminService upsert bug, we already upserted it as chairman?
                // Wait, adminService.quickCreateChairman hardcodes 'chairman'!
                // I need to change quickCreateChairman to accept role.
...
"""
content = content.replace('title="চেয়ারম্যান নিয়োগ"', 'title="চেয়ারম্যান নিয়োগ"')
